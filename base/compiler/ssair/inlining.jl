...
function InliningTodo(mi::MethodInstance, ir::IRCode)
    ...
    inline_linetable = Vector{LineInfoNode}(undef, length(ir.linetable))
    for i = 1:length(ir.linetable)
        entry = ir.linetable[i]
        if entry.inlined_at === 0 && entry.method === mi.def
            entry = LineInfoNode(mi, entry.file, entry.line, entry.inlined_at)
        end
        inline_linetable[i] = entry
    end
    return InliningTodo(mi, ResolvedInliningSpec(inline_linetable, ir, linear_inline_eligible(ir)))
end
...
function find_inferred(mi::MethodInstance, @nospecialize(atypes), caches::InferenceCaches, @nospecialize(rettype))
