...
isdefined(Main, :is_atrisk_type) || @eval Main include("testhelpers/methodanalysis.jl")
...
@testset "Base.require vulnerabilities" begin
    ...
    for m in methods(Base.require)
        @test_broken isempty(remove_unlikely_methodinstances(atrisk_triggers(m, first.(mivulnerabilities_exported))))
